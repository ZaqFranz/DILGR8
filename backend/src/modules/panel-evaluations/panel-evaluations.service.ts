...
export interface ApplicantScoreCriterionColumn {
  ...
  categoryId: string;
  name: string;
  // The raw max (0-maxScore), not a weighted figure - unlike a category,
  // a criterion has no weightPercent of its own; it only ever contributes
  // to its parent category's weighted total (see weightedCategoryScore).
  maxScore: number;
}
...
export interface ApplicantScoreRow {
  ...
  // panelists actually scored this application - the report/summary view
  // an admin sees (see Report Summary page): combined across every
  // panelist, never any one panelist's individual number.
