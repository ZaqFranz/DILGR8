...
| Method | Path | Auth | Body | Notes |
|---|---|---|---|---|
| POST | `/` | applicant | `{ jobPostingId }` | Submits an application. Validates: profile exists, posting is open and within its 10-day window, not already applied. If the posting has any `requiredEligibilityTypes`, 400s unless the applicant has `hasEligibility: true` **and** `eligibilityType` is one of them (message lists the accepted types); separately, if `hasEligibility` is true, an `ELIGIBILITY_PROOF` document must have been uploaded regardless of whether the posting requires eligibility. `positionLevel === PROMOTIONAL` postings have no extra document requirement at submission time - `DESIGNATION_ORDER`/`IPCR` are collected once at registration (optional, per the official documents checklist) rather than gated per application, see [decisions.md](./decisions.md). |
