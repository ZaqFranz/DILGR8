...
| GET | `/applicant-scores` | ADMIN | — | The Categories page's "Applicant Scores" overview: every `FOR_INTERVIEW`/`QUALIFIED`/`NOT_QUALIFIED` application **across all job postings** that has at least one submitted `PanelEvaluation` (unscored applications are excluded). Unlike `/tabulation/:jobPostingId`, this is not scoped to one posting. Returns `{ categories: [{ id, name, weightPercent }], rows: [{ applicationId, applicantName, jobPostingTitle, perCategory: Record<categoryId, number \| null>, total: number \| null, panelistsSubmitted }] }` — `perCategory` is, per application, each panelist's **weighted** contribution for that category (their raw subtotal across the category's own criteria, normalized to `weightPercent` - see below), then averaged across every panelist who scored the application; `total` is the same averaging over each panelist's full weighted grand total. Both computed against every currently-active category/criterion. `categories[].weightPercent` (not `maxScore`) is the ceiling the `perCategory` numbers can reach, so that's what column headers should show. Rows are returned unranked/unsorted — ranking by a chosen column (overall total or one specific category) is done client-side in `ApplicantScoresModal`, since which column to rank by is an interactive display choice, not fixed data. |
