...
A request body that isn't valid JSON gets the same envelope with `code: "VALIDATION_ERROR"` and a 400 (added 2026-08-22, audit remediation - previously fell through to a generic 500).

Authenticated routes require `Authorization: Bearer <accessToken>`. Since 2026-08-22 (audit remediation), every authenticated request also re-checks the token's embedded `tokenVersion` against the user's current DB row (401 if it doesn't match) - see [architecture.md § Authentication & session revocation](./architecture.md#authentication--session-revocation).
...
| GET | `/me` | applicant | — | Lists the caller's applications with their job posting, `scoreSourceApplication` (which of the caller's other applications this one's interview score is carried over from, if any), and `scoreInheritingApplications` (added 2026-08-21 - the reverse: other applications of the caller's that carry over *this* one's score, each with its own `status`). Both sides exist because each application's pipeline `status` still advances independently per posting even though the interview score is shared - `MyApplicationsPage.tsx` uses `scoreInheritingApplications` to tell the applicant on the canonical (actually-interviewed) application that a sibling exists and how far along it currently is, instead of that card looking stuck while a sibling has quietly moved on to Compliance/Oath-Taking. |
...
| PATCH | `/:id/hire` | ADMIN | — | Marks the oath-taking ceremony completed. 400 unless `status === "FOR_OATH_TAKING"`. Sets `status: HIRED` and stamps `hiredAt` - terminal for this pipeline segment; Onboarding itself (videos, pre/post evaluations) is future work. Since 2026-08-21, also auto-closes every other of this applicant's still-`OPEN_APPLICATION_STATUSES` applications to `NOT_SELECTED` (system-generated `rejectionRemarks` naming the posting they lost to and its Salary Grade, `APPLICATION_NOT_SELECTED` audit entry, regret email each) - a person can't hold two positions, so hiring them on one closes out any others they were still pursuing. An already-terminal sibling (e.g. `WITHDRAWN`) is left untouched. Unchanged since (same-day follow-up, still 2026-08-21): the frontend's "Assign Position" picker (`EvaluateApplicantsPage`'s `ApplicantGroupSummaryRow`/`AssignPositionModal`) calls this exact endpoint with whichever application id the admin picks from a neutral list once an applicant has 2+ applications at `FOR_OATH_TAKING` - no dedicated bulk/multi-posting endpoint was added, this single-application action was already the right primitive. See [architecture.md § Multi-posting applications & panel score inheritance](./architecture.md#multi-posting-applications--panel-score-inheritance-phase-1). |
...
| PATCH | `/:id` | `{ email?, role?, name? }` | No password field — password changes go through `POST /:id/reset-password` below (self-service change is `PATCH /api/auth/me/password` instead). Any update bumps the target's `tokenVersion`, forcing every token they're currently holding to re-authenticate (added 2026-08-22 - see [architecture.md § Authentication & session revocation](./architecture.md#authentication--session-revocation)). |
| DELETE | `/:id` | — | 400 if you try to delete your own account, or (added 2026-08-22, audit remediation) if the target has at least one `Application` row - deleting a user cascades to their `Applicant` profile and everything under it (`PanelEvaluation`/`ApplicationComplianceItem`/`Document`/`ApplicantGroupMember`), which would otherwise silently erase real hiring history with no soft-delete to recover it; job postings they created or applications they evaluated are kept, with the FK set to null (see [decisions.md](./decisions.md)). |
...
| Method | Path | Auth | Body | Notes |
|---|---|---|---|---|
| GET | `/my-queue` | PANEL | — | Applications with `status: FOR_INTERVIEW` across every posting the caller is assigned to, each including the caller's own existing evaluation (if any) so the frontend can pre-fill an in-progress form. Each entry's `applicant` now also includes `id` (previously only `firstName`/`lastName`) so the frontend can pass it into `GET /applicants/:id/documents` for the "View PDS" button. Excludes any application whose `scoreSourceApplicationId` is set (added 2026-08-21) - it already has an effective score inherited from another of the applicant's applications, so no panelist is ever asked to score it again. Also runs a self-healing repair pass (added 2026-08-21, fixes a "duplicate interviews" bug) before returning: an application can still be sitting here unlinked if the applicant applied to two postings *before either was scored* - the repair batch-checks every remaining candidate's applicant for one that's since been scored elsewhere and, if found, links it (same effect as the reactive link below) and excludes it from the response instead of leaving it to be scored twice. Each entry also carries `otherApplications: { jobPostingTitle }[]` (same-day follow-up, still 2026-08-21) - originally every other application (any status) this applicant had on file, tightened same day to exactly the set `linkSiblingScoreSources()` will actually link once this one is scored (open, no evaluations of its own, not already pointed elsewhere), resolved via one batched query. `MyInterviewsPage` (also same-day, still 2026-08-21) now dedupes the raw queue to one row per applicant client-side (`groupByApplicant()`, rendering only the earliest-submitted entry) rather than showing one row per posting, and uses this field for the rendered row's "This score will also count for: ..." line - the endpoint itself is unchanged otherwise, still returning one entry per still-scoreable application; the dedup and the tightened field meaning both live in what the frontend does with the response. |
| PATCH | `/:applicationId` | PANEL | `{ remarks?, scores: [{ criterionId, score }] }` | Upserts the caller's own evaluation for that application (one per panelist per application — re-submitting overwrites, mirroring `applications.sift`'s upsert-on-resubmit pattern). `criterionId` here is a **Criterion** (the leaf) id, not a Category id - a panelist scores every individual criterion on its own **raw** scale (0 to that criterion's `maxScore`), never a category as one number; how those raw scores translate into the category's actual weighted contribution happens downstream (see "Weighted scoring" below), not at submission time. 400 if the application isn't `FOR_INTERVIEW`, its `examinationScore` is still `null` (PQE score not yet recorded — see below), its `scoreSourceApplicationId` is already set (added 2026-08-21 - this application's score is carried over from another of the applicant's applications; scoring it directly isn't allowed, and it never appears in `GET /my-queue` in the first place, see below), **or a live re-check right before the write finds the applicant has since been scored on another application anyway** (added 2026-08-21, `PanelEvaluationsRepository.reconcileScoreSource()` - closes the same race as `/my-queue`'s repair pass above for two panelists submitting around the same time; auto-links this application to the newly-discovered canonical one before refusing, same error message as the stored-field check), a score is missing for any active criterion (message names it as `"<Category name> - <Criterion name>"`), or a score exceeds that criterion's own `maxScore` (the spec's mandatory-field + threshold rules); 403 if the caller isn't assigned to that application's posting. On success, propagates the new score to every other of the applicant's open applications that don't already have their own evaluation or an existing score source (`PanelEvaluationsRepository.linkSiblingScoreSources()`), so a multi-posting applicant is only ever interviewed once. |
