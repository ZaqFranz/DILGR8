...
**Status flow**: `Application.status` gained `FOR_INTERVIEW`, sitting between submission/sifting and the existing `QUALIFIED`/`NOT_QUALIFIED` finalization. `PATCH /applications/:id/schedule-interview` (admin) is the only way in; `PATCH /applications/:id/evaluate` (admin, unchanged) is still the only way out, and deliberately has no precondition on panel-scoring completeness — an admin can finalize early. The frontend is where that gets guarded: `EvaluateApplicantsPage` fetches `GET /panel-evaluations/tabulation/:jobPostingId` alongside the applications list and shows a `panelistsSubmitted < panelistsAssigned` warning inside `EvaluationRow`'s expanded evaluate form before the admin saves a decision.
