...
- **Admin UI covers Dashboard, Job Management, Users Management, Evaluate Applicants, Interview Panel, Evaluation Criteria, and History of Logs.** There's still no screen for admins to validate Eligibility=N applicants specifically (the Sifting decision on `EvaluateApplicantsPage` covers the general pass/fail call, but doesn't surface the Eligibility=N red-flag from registration as its own checklist item).
...
- **No email for the `WITHDRAWN` stage.** There's no applicant-facing withdraw action yet, so there's no transition to hook a notification into.
