...
### `applications`
Join between `applicants` and `job_postings`, unique on `(applicantId, jobPostingId)` — an applicant may apply to several postings but only once each. The full pipeline: `SUBMITTED` → `UNDER_SIFTING` (set automatically by `ApplicationsRepository.create()` the instant an application is submitted - there's no separate "start sifting" step) → `QUALIFIED`/`NOT_QUALIFIED` (the Sifting phase's pass/fail call against the posting's qualification standards, via `PATCH /api/applications/:id/sift`, 400 unless currently `UNDER_SIFTING`) → PQE score recorded on a `QUALIFIED` application (`POST /api/applications/import-exam-scores`, an Excel import matched by applicant name - doesn't change `status`) → `FOR_INTERVIEW` (`PATCH /api/applications/:id/schedule-interview`, 400 unless `status === QUALIFIED` **and** `examinationScore` is set; also stamps `interviewScheduledAt`/`interviewVenue`/`interviewAttire`/`interviewNotes` from the admin-supplied `ScheduleInterviewDto`, so the notification email can tell the applicant when/where to show up and what to wear) → interview panel scoring (`panel_evaluations`/`panel_scores` below). `WITHDRAWN` can be reached from any of `SUBMITTED`/`UNDER_SIFTING`/`QUALIFIED`/`FOR_INTERVIEW` via the applicant's own `PATCH /api/applications/:id/withdraw` (stamps `withdrawnAt`) - it's the one transition the applicant themself triggers rather than an admin/panelist, and it's terminal: `NOT_QUALIFIED` and `WITHDRAWN` itself can't be withdrawn from since there's nothing left to withdraw. There's no status past `FOR_INTERVIEW` yet — the Deliberation phase (final RD decision) is future work, tracked in `docs/project-memory.md`.
